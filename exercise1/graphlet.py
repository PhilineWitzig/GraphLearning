...
def compute_graphlets(datasets, names):
    ...
    If specified in config file, the computed vectors are stored for later use.
    ...
    if not os.path.exists(os.path.join(os.getcwd(), "data")) and config.PREPROC_STORE:
    ...
    for i, dataset in enumerate(datasets):
        graphlet = np.zeros([len(dataset), config.GRAPHLET_VECTOR_SIZE])
        ...
        if config.PREPROC_STORE:
            np.save(os.path.join(os.getcwd(),
                                 "data", "graphlet", "dataset_" + names[i] + ".npy"), graphlet)
...
def graph_to_hist(x):
    ...
    hist = np.zeros(config.GRAPHLET_VECTOR_SIZE)  # histogram vector for graph x
